...
#define LOG_BUFFER_SIZE 256 /* 收发缓存大小 */
static uint8_t tx_buf[LOG_BUFFER_SIZE]; /* 发送缓冲 */
static uint8_t rx_buf[LOG_BUFFER_SIZE]; /* 接收缓冲 */

static queue_info_t rx_q; /* 接收队列 队列组件框架已提供 */
...
static void log_device_init(void)
{
	...
	dma_param.number = LOG_BUFFER_SIZE; /* DMA大小 */
	...
	queue_init(&rx_q, 1, rx_buf, LOG_BUFFER_SIZE); /* 初始化队列 */
...
void USART0_IRQHandler(void)
{
	if (usart_interrupt_flag_get(USART0, USART_INT_FLAG_RT) != RESET) {
		uint16_t cur_pos = LOG_BUFFER_SIZE - dma_transfer_number_get(DMA0, DMA_CH4); /* DMA当前指针 */
		uint16_t cur_recieve = (cur_pos - pre_pos) & (LOG_BUFFER_SIZE - 1); /* 计算接收长度 */
